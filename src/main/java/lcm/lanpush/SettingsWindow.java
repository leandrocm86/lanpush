...
public class SettingsWindow implements PropertyChangeListener {
    ...
    private static final String HINT_MINIMIZE_TO_TRAY = "Whether to move the app's window to the system tray.";
    private static final String HINT_ON_RECEIVE_NOTIFY = "Whether to display a notification when a message is received (only works when the app is on the system tray).";
    ...
    private void createContentPane() {
        ...
        optionPanes.add(createOptionPanel("Log file path", createLogFilePanel(), 90, HINT_LOG_PATH));
    ...
    private void logPathChanged() {
        ...
        try {
            Filer.getForWriting(logPathOption.getText());
            OLog.info("Log path selected: %s", logPathOption.getText());
            config.setLogPath(logPathOption.getText());
        }
        catch (IllegalArgumentException e) {
            JOptionPane.showMessageDialog(null, "LANPUSH cannot write to the selected path. " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void initializeValues() {
        ...
        if (!config.minimizeToTray())
            onReceiveNotifyOption.setEnabled(false);
    ...
    private void setUpdateEvents() {
        ...
        minimizeToTrayOption.addMouseListener(new ConfigChanged(() -> {config.setMinimizeToTray(minimizeToTrayOption.isSelected()); onReceiveNotifyOption.setEnabled(minimizeToTrayOption.isSelected());}));
        onReceiveNotifyOption.addMouseListener(new ConfigChanged(() -> config.setOnReceiveNotify(onReceiveNotifyOption.isSelected())));
        startMinimized.addMouseListener(new ConfigChanged(() -> config.setStartMinimized(startMinimized.isSelected())));
        ...
        onReceiveRestoreOption.addMouseListener(new ConfigChanged(() -> config.setOnReceiveRestore(onReceiveRestoreOption.isSelected())));
    }

    private class ConfigChanged implements FocusListener, MouseListener {
